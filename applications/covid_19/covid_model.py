...
def build_covid_model(country: str, update_params: dict):
    ...
    for compartment in is_infectious:
        model_parameters['within_' + compartment] *= \
            float(model_parameters['n_compartment_repeats'][compartment])
    ...
    total_infectious_times = \
        sum([model_parameters['compartment_periods'][comp] for comp in is_infectious])
    ...
    output_connections.update(
        create_fully_stratified_incidence_covid(
            model_parameters['stratify_by'], model_parameters['all_stratifications'], model_parameters
        )
    )
