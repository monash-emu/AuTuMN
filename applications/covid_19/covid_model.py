...
COUNTRIES = (AUSTRALIA, PHILLIPINES)
...
def build_covid_model(country: str, update_params: dict):
    ...
    for compartment in infected_compartment_types:
        compartments, infectious_compartments, init_pop = replicate_compartment(
            n_repeats,
            compartments,
            compartment,
    ...
    flows = add_infection_flows([], n_repeats)
    for compartment in infected_compartment_types:
        flows = add_sequential_compartment_flows(flows, n_repeats, compartment)
    flows = add_transition_flows(
        flows, n_repeats, Compartment.EXPOSED, Compartment.PRESYMPTOMATIC, "within_exposed"
    )
    ...
    flows = add_transition_flows(
        flows, n_repeats, Compartment.PRESYMPTOMATIC, Compartment.INFECTIOUS, "to_infectious"
    )
    ...
    if "clinical" in model_parameters["stratify_by"] and model_parameters["clinical_strata"]:
        _covid_model, model_parameters = stratify_by_clinical(
            _covid_model, model_parameters, compartments
        )

    # Add fully stratified incidence to output_connections
    output_connections.update(
        create_fully_stratified_incidence_covid(
            model_parameters["stratify_by"], model_parameters["all_stratifications"], n_repeats
        )
    )
