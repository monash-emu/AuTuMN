...
def build_rmi_timevariant_cdr(cdr_multiplier):
    cdr = {1950.0: 0.0, 1980.0: 0.10, 1990.0: 0.1, 2000.0: 0.2, 2010.0: 0.3, 2015: 0.3}
...
def build_rmi_model(update_params={}):
    ...
    external_params = {  # run configuration
        ...
        "latency_adjustment": 2.0,  # used to modify progression rates during calibration
        ...
        "acf_ltbi_sensitivity": 0.9,
        "acf_ltbi_efficacy": 0.85,  # higher than ipt_efficacy as higher completion rate
    ...
    integration_times = numpy.linspace(
        external_params["start_time"], external_params["end_time"], 4 * n_iter
    ).tolist()
    ...
    def mortality_derived_output(model):
        ...
        for comp_ind in model.infectious_indices["all_strains"]:
            ...
            flow_index = model.death_flows[
                model.death_flows.origin == model.compartment_names[comp_ind]
            ].index
            param_name = model.death_flows.parameter[flow_index].to_string().split("    ")[1]
            mortality_rate = model.get_parameter_value(param_name, 2019.0)
    ...
    if "age" in stratify_by:
        ...
        pop_morts = get_pop_mortality_functions(
            input_database, age_breakpoints, country_iso_code="FSM"
        )
    ...
    if "organ" in stratify_by:
        props_smear = {
            ...
            "smearneg": 1.0 - (external_params["prop_smearpos"] + 0.3),
            "extrapul": 0.3,
        }
    ...
    if "diabetes" in stratify_by:
        ...
        progression_adjustments = {"has_diabetes": 3.18, "no_diabetes": 1.0}

        _tb_model.stratify(
            "diabetes",
            ["has_diabetes", "no_diabetes"],
            [],
            verbose=False,
    ...
    if load_model:
        ...
        if load_mcmc:
            ...
            for sc in scenarios_to_load:
                ...
                model_dict = load_model_scenario(
                    str(sc), database_name="outputs_11_27_2019_13_12_43.db"
                )
                models.append(DummyModel(model_dict))
    else:
        t0 = time()
        models = run_multi_scenario(scenario_params, 2000.0, build_rmi_model)
