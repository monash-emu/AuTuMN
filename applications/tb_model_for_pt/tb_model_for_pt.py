...
def build_model(update_params={}):
    ...
    external_params = {  # run configuration
        ...
        "rr_transmission_province": 0.9,  # reference: rural
        # IPT
        "n_ipt": 0.0,
        "n_ipt_age_0": 0.0,
        "n_ipt_age_5": 0.0,
        ...
        "ipt_efficacy": 0.9,  # based on intention-to-treat
        ...
        "acf_sensitivity": 0.8,
        "acf_ger_switch": 0.0,
        "acf_non-ger_switch": 0.0,
        "acf_rural_switch": 0.0,
        "acf_province_switch": 0.0,
        "acf_urban_switch": 0.0,
    }
    ...
    def early_ipt_flow_func(model, n_flow):
        no_age_stratifications = {
            key: value for key, value in model.all_stratifications.items() if key != "age"
        }
        n_pop_subgroups = 1.0
        ...
        index_late_cpt = model.compartment_names.index(
            model.transition_flows.origin[n_flow].replace("early_latent", "late_latent")
        )

        time_step = 0.0 if len(model.times) < 2 else (model.times[-1] - model.times[-2])

        n_early_latent = model.compartment_values[index_early_cpt]
        n_late_latent = model.compartment_values[index_late_cpt]
        if n_early_latent + n_late_latent > 0.0:
            return (
                n_early_latent
                / (n_early_latent + n_late_latent)
                / n_pop_subgroups
                * time_step
    ...
    def late_ipt_flow_func(model, n_flow):
        ...
        index_early_cpt = model.compartment_names.index(
            model.transition_flows.origin[n_flow].replace("late_latent", "early_latent")
        )
    ...
    prop_to_rate = convert_competing_proportion_to_rate(
        1.0 / external_params["untreated_disease_duration"]
    )
    ...
    tb_control_recovery_rate = lambda t: detect_rate(t) * (
        mongolia_tsr(t) + external_params["reduction_negative_tx_outcome"] * (1.0 - mongolia_tsr(t))
    )
    ...
    if "age" in stratify_by:
        ...
        ipt_by_age = {"n_ipt": {}}
        for age_break in age_breakpoints:
            ipt_by_age["n_ipt"][str(age_break) + "W"] = external_params[
                "n_ipt_age_" + str(age_break)
            ]
        ...
        age_mixing = numpy.ones(len(age_breakpoints) ** 2).reshape(
            (len(age_breakpoints), len(age_breakpoints))
        )
    ...
    if "housing" in stratify_by:
        props_housing = {"ger": 0.45, "non-ger": 0.55}
        raw_relative_risks = {"ger": external_params["rr_transmission_ger"], "non-ger": 1.0}
        scaled_relative_risks = scale_relative_risks_for_equivalence(
            props_housing, raw_relative_risks
        )
        ...
        _tb_model.stratify(
            ...
            ["ger", "non-ger"],
            [],
            requested_proportions=props_housing,
            verbose=False,
    ...
    if "location" in stratify_by:
        props_location = {"rural": 0.32, "province": 0.16, "urban": 0.52}
        raw_relative_risks_loc = {
            "rural": 1.0,
...
def run_scenario(scenario_params={"start_time": 2020.0}, baseline_was_run=True):
    if "start_time" not in scenario_params.keys():
        scenario_params["start_time"] = 2020.0
...
def get_model_output(run_model):
    ...
    req_times = {"prevXinfectiousXamong": [2049.0]}
    pp = post_proc.PostProcessing(
        run_model, requested_outputs=req_output, requested_times=req_times, multipliers=multipliers
    )
