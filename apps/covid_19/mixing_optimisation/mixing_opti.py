...
def make_scenario_func(
        ...
        mode: str = AGE_MODE,
        final_mixing: float = 1.0,):  # a function that returns a scenario dictionary based on decision variables

    elderly_mixing_reduction = root_params["default"]["elderly_mixing_reduction"]
...
def has_immunity_been_reached(run_model, phase_2_end_index: int) -> bool:
    ...
    two_weeks_days = 14
    incidence_vals = [run_model.derived_outputs["incidence"][i] for i in time_indices[two_weeks_days:]]
...
def make_objective_func(duration):
    ...
    def objective_func(run_model, decision_vars):
        ...
        years_of_life_lost = sum(run_model.derived_outputs["years_of_life_lost"][phase_2_start_idx:])
