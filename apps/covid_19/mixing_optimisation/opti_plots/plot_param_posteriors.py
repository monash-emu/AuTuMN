...
param_info = {
    ...
    "start_time": {"name": "model start time", "range": [0.0, 40.0]},
    "npi_effectiveness.other_locations": {"name": "alpha", "range": [0.5, 1.0]},
    "compartment_periods_calculated.exposed.total_period": {
        "name": "incubation time",
        "range": [3.0, 7.0],
    },
    "compartment_periods_calculated.active.total_period": {
    ...
    "hospital_props_multiplier": {"name": "hosp. prop. multiplier", "range": [0.5, 2.0]},
    "microdistancing.parameters.c": {"name": "microdistancing (inflection)", "range": [90, 152]},
    "microdistancing.parameters.sigma": {"name": "microdistancing (final)", "range": [0.6, 1.0]},
}
...
for i in range(9):
    ...
    if age_index == "8":
        age_group = "80+"
    param_info[pollan_priors[i]["param_name"]] = (
        {
            "name": "IFR age " + age_group,
            "range": pollan_priors[i]["distri_params"],
            "xticks": pollan_priors[i]["distri_params"],
            "xlabels": [
                "{:.2e}".format(b).split("e")[0] for b in pollan_priors[i]["distri_params"]
            ],
            "multiplier": "(.e"
            + "{:.2e}".format(pollan_priors[i]["distri_params"][0]).split("e")[1]
            + ")",
        },
    )
    if age_index == "0":
        param_info[pollan_priors[i]["param_name"]][0]["multiplier"] = "(.e-06)"
        param_info[pollan_priors[i]["param_name"]][0]["xlabels"][1] = str(
            10.0 * float(param_info[pollan_priors[i]["param_name"]][0]["xlabels"][1])
        )
...
def get_param_values_by_country(country, calibration_folder_name, burn_in=0):
    ...
    calibration_output_path = (
        "../../../../data/outputs/calibrate/covid_19/" + country + "/" + calibration_folder_name
    )
    mcmc_tables, _, _ = collect_all_mcmc_output_tables(calibration_output_path)
...
def get_all_param_values(calibration_folder_name):
    ...
    for country in OPTI_REGIONS:
        param_values[country] = get_param_values_by_country(
            country, calibration_folder_name, burn_in_by_country[country]
        )
...
def plot_param_posteriors(param_values, param_info={}):
    ...
    for param_name in list(param_values["belgium"].keys()):
        ...
        for country in OPTI_REGIONS:
            ...
            low_95, low_50, med, up_50, up_95 = quantile(values, q=(0.025, 0.25, 0.5, 0.75, 0.975))
        ...
        if i_col == 0:
            ...
            axs[i_row, i_col].set_yticklabels(
                [c.title().replace("United-Kingdom", "UK") for c in OPTI_REGIONS]
            )
        else:
