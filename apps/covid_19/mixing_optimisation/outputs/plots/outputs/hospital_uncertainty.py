...
FIGURE_PATH = os.path.join(BASE_PATH, "apps", "covid_19", "mixing_optimisation",
                           "outputs", "plots", "outputs", "figures", "hospital_uncertainty")
...
def plot_multiscenario_uncertainty(uncertainty_df, mode, axis, output, country):
    ...
    for i, sc_idx in enumerate(sc_idx_to_plot):
        ...
        if sc_idx > 0:
            axis.fill_between(times, data[sc_idx]['quantiles'][0.025], data[sc_idx]['quantiles'][0.975], facecolor="lightsteelblue")
            axis.fill_between(times, data[sc_idx]['quantiles'][0.25], data[sc_idx]['quantiles'][0.75], facecolor="cornflowerblue")
            axis.plot(times, data[sc_idx]['quantiles'][0.50], color="navy")
        else:
            axis.fill_between(times, data[sc_idx]['quantiles'][0.025], data[sc_idx]['quantiles'][0.975], facecolor="plum")
            axis.fill_between(times, data[sc_idx]['quantiles'][0.25], data[sc_idx]['quantiles'][0.75], facecolor="mediumorchid")
...
def plot_multicountry_multiscenario_uncertainty(uncertainty_dfs, output, mode):
    ...
    titles_down = ["epidemic", "minimising deaths", "minimising YLLs", "minimising deaths", "minimising YLLs"]
    text_size = 23
