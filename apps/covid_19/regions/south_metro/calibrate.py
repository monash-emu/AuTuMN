...
TARGET_OUTPUTS = add_standard_victoria_targets([], targets, Region.SOUTH_METRO)

PAR_PRIORS = provide_default_calibration_params(("time.start", "contact_rate"))
PAR_PRIORS = add_standard_victoria_params(PAR_PRIORS, Region.SOUTH_METRO)
PAR_PRIORS = add_dispersion_param_prior_for_gaussian(PAR_PRIORS, TARGET_OUTPUTS)


def run_calibration_chain(max_seconds: int, run_id: int, num_chains: int):
    base.run_calibration_chain(
        max_seconds, run_id, num_chains, Region.SOUTH_METRO, PAR_PRIORS, TARGET_OUTPUTS,
    )
