...
from apps.covid_19.vaccine_optimisation.vaccine_opti import initialise_opti_object
...
def write_scenario_yml_file(country, decision_vars, sc_start_index=None):
    ...
    # uniform scenario
    uniform_vars = [1. / 8. for _ in range(16)] + [decision_vars[-1]]

    # elderly scenario
    elderly_vars = 2 * ([0. for _ in range(6)] + [.5, .5]) + [decision_vars[-1]]
    ...
    for i, _vars in enumerate(sc_decision_vars):
        ...
        param_file_path = os.path.join(BASE_PATH, "apps", "covid_19", "regions", country_folder_name, "params", f"scenario-{sc_index}.yml")
