...
def get_unstratified_parameter_values(params):
    for param_name in ['stabilisation_rate', 'early_activation_rate', 'late_activation_rate']:
        params[param_name] = 365.25 * params['age_specific_latency'][param_name]['unstratified'] if "age" not in params['stratify_by'] else 1.
    return params
...
def edit_adjustments_for_diabetes(model, adjustments, age_breakpoints, prop_diabetes, rr_progression_diabetes):
    diabetes_scale_up = tanh_based_scaleup(b=.05, c=1980, sigma=0., upper_asymptote=1.)
...
def make_age_diabetes_scaleup_func(final_prop_diabetes, scale_up_func, rr_progression_diabetes, original_param_val):
    def diabetes_func(t):
        return (1. - scale_up_func(t) * final_prop_diabetes * (1. - rr_progression_diabetes)) * original_param_val
