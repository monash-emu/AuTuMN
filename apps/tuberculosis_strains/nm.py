...
def build_model(params: dict) -> CompartmentalModel:
    ...
    # init_pop = {
    #     Compartment.EARLY_LATENT: Parameter("initial_early_latent_population"),
    #     Compartment.LATE_LATENT: Parameter("initial_late_latent_population"),
    #     Compartment.INFECTIOUS: Parameter("initial_infectious_population"),
    #     Compartment.DETECTED: Parameter("initial_detected_population"),
    #     Compartment.ON_TREATMENT: Parameter("initial_on_treatment_population"),
    #     Compartment.RECOVERED: 0,
    # }
    # sum_init_pop = sum(init_pop.values())
    # init_pop[Compartment.SUSCEPTIBLE] = Parameter("start_population_size") - sum_init_pop
    ...
    model.add_death_flow(
        "treatment_death",
        Parameter("treatment_death_rate"),
        Compartment.ON_TREATMENT,
    )
    ...
    model.add_transition_flow(
        name="amplification",
        fractional_rate=Parameter("amplification_rate"),
    ...
    model.add_infection_frequency_flow(
        ...
        contact_rate=Parameter("reinfection_rate"),
...
def _get_strat_param_dict(params, key):
    layers = key.split('.')
    p_dict = params
    for l in layers:
        p_dict = p_dict[l]
    
    return {s: Parameter(key + f'.{s}') for s, v in p_dict.items()}
...
def _build_age_strat(params: dict, uni_death_flow_names: list):
    ...
    age_strat.add_flow_adjustments(
        "stabilisation", _adjust_all_multiply(_get_strat_param_dict(params,"stabilisation_rate_stratified.age"))
    )
    age_strat.add_flow_adjustments(
        "early_activation", _adjust_all_multiply(_get_strat_param_dict(params,"early_activation_rate_stratified.age"))
    )
    age_strat.add_flow_adjustments(
        "late_activation", _adjust_all_multiply(_get_strat_param_dict(params,"late_activation_rate_stratified.age"))
    )
...
def _build_vac_strat(params):
    ...
    vac_strat.add_flow_adjustments(
        "infection",
        {
            ...
            "vaccinated": Multiply(Parameter("bcg.rr_infection_vaccinated")),
        },
    )
    vac_strat.add_flow_adjustments(
        "treatment_early",
    ...
    def time_varying_vaccination_coverage(t, params):
        return params["bcg"]["coverage"] if t > params["bcg"]["start_time"] else 0.0

    def time_varying_unvaccinated_coverage(t, params):
        return 1 - params["bcg"]["coverage"] if t > params["bcg"]["start_time"] else 1.0

    vac_strat.add_flow_adjustments(
        "birth",
        {
            "unvaccinated": Multiply(TimeParamFuncManager(time_varying_unvaccinated_coverage)),
            "vaccinated": Multiply(TimeParamFuncManager(time_varying_vaccination_coverage)),
        },
    )
    return vac_strat
