...
SOURCE_MATRICES_PATHS = {
    ...
    "conmat": os.path.join(INPUT_DATA_PATH, "social-mixing", "conmat_outputs"),
}
...
def load_socialmixr_matrices(proxy_country_iso3, requested_locations):
    ...
    """
    available_source_countries = os.listdir(SOURCE_MATRICES_PATHS['socialmixr'])
    assert (
    ...
    for i, location in enumerate(requested_locations):
        matrix_path = os.path.join(SOURCE_MATRICES_PATHS['socialmixr'], proxy_country_iso3, f"{location}.csv")
        msg = f"Could not find the required file {matrix_path}"
        assert f"{location}.csv" in os.listdir(
            os.path.join(SOURCE_MATRICES_PATHS['socialmixr'], proxy_country_iso3)
        ), msg
...
def get_conmat_matrix(iso3: str, setting: str, age_groups: list):
    """
    Returns a mixing matrix for a given iso3 code, and specific setting, using the matrices generated by the conmat R package
    """

    # "other_locations" becomes "other"  /  "all_locations" becomes "all"
    setting = setting.split("_locations")[0]
    ...
    for _, row in conmat_data.iterrows():
        age_from = int(row['age_group_from'][1:].split(",")[0])
        age_to = int(row['age_group_to'][1:].split(",")[0])
        matrix[age_groups.index(age_from), age_groups.index(age_to)] = row['contacts']
    return matrix


def get_matrices_from_conmat(iso3, age_groups):
    """
    Create mixing matrices for all settings, for the requested iso3 code and age_groups

    Args:
        iso3 (bool): _description_
        age_groups (_type_): _description_

    Returns:
        _type_: _description_
    """
    mixing_matrices = {setting: get_conmat_matrix(iso3, setting, age_groups) for setting in LOCATIONS if setting != "all_locations"}
