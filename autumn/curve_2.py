...
def test_a(a, x_min, x_max, x_peak, bound_low, bound_up):
    ...
    x_peak is one of x_min or x_max. At this point, the curve reaches an extrema equal to bound_low or bound_up
...
def get_spare_fit(indice, x_peak, bound, side, f, cut_off_dict, bound_low, bound_up, a_init, a_f, x, y):
    ...
    reach an extrema on the bound that was overcome. However, in some cases, this new fit presents a singularity where
...
def scale_up_function(x, y, method=3, smoothness=1.0, bound_low=None, bound_up=None,):
    """
    ...
    Input:
        ...
        method: Select an interpolation method. Methods 1, 2 and 3 use cubic interpolation defined step by step.
			...
			5: Uses an adaptive algorithm producing either an interpolation or an approximation, depending on the value of
			the smoothness. This method allows for consideration of limits that are defined through bound_low and bound_up.
			See detailed description of this method in the code after the line "if method == 5:"
		smoothness, bound_up, bound_low are only used when method=5
		smoothness: Defines the level of smoothness of the curve. The minimum value is 0.0 and leads to an interpolation.
    ...
    def derivatives(x, y):
        ...
        v = np.zeros(len(x))  # initializes all zeros
    ...
    if method == 1:
		...
		3. If bounds are defined, we detect potential sections where they are overcome and we use the following method to
			adjust the fit:
			...
			3.3. If g still presents irregularities, we repeat the same process from 3.1. on the interval [x_(i-1), x_j] or
				[x_i, x_(j+1)], depending on which side of the interval led to an issue
		"""
		...
		if (bound_low is not None) or (bound_up is not None):
			if bound_low is not None:
				assert min(y) >= bound_low, "y contains values < bound_low. Please adjust bound_low or remove constraint."
			...
			def cut_off(indice, bound_low, bound_up, sign):
				...
				while go == 1:
					...
					if (indice + k) == 0:
						y_k = y[0]
					elif (indice+k) == len(x) - 2:
						y_k = a_f[0] + a_f[1] * x[indice+k] + a_f[2] * x[indice+k] ** 2 + a_f[3] * x[indice+k] ** 3
					elif (indice+k) == len(x) - 1:
						y_k = y[-1]
					else:
						y_k = f(x[indice+k])
				...
				if t1 == 0:  # there is something wrong here
					spare = get_spare_fit(indice, x_peak, bound, 'left', f, cut_off_dict, bound_low, bound_up, a_init, a_f, x, y)
					a1 = spare['a']
					indice_first = indice - spare['cpt']
				t2 = test_a(a2, x_peak, x1, x_peak, bound_low, bound_up
							)
				if t2 == 0:  # there is something wrong here
					spare = get_spare_fit(indice, x_peak, bound, 'right', f, cut_off_dict, bound_low, bound_up, a_init, a_f,x ,y)
					a2 = spare['a']
					indice_next = indice + 1 + spare['cpt']

				out = {'a1': a1, 'a2': a2, 'x_peak': x_peak, 'indice_first': indice_first, 'indice_next': indice_next}
				return (out)
