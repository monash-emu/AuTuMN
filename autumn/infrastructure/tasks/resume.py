...
from autumn.calibration.calibration import Calibration
...
def resume_calibration_task(
    run_id: str, base_run_id: str, runtime: float, num_chains: int, verbose: bool = False
):
    ...
    keys_to_dl = [
        k
        for k in all_cal_data_keys
        if any([t in k for t in [".pkl", "mcmc_run.parquet", "mcmc_params.parquet"]])
    ]
    ...
    with Timer(f"Resuming {num_chains} calibration chains"):
        args_list = [(runtime, chain_id, verbose) for chain_id in range(num_chains)]
        try:
            run_parallel_tasks(resume_calibration_chain, args_list, False)
            cal_success = True
...
def resume_calibration_chain(runtime: float, chain_id: int, verbose: bool):
    """
    ...
    try:
        ...
        rcal = Calibration.from_existing(
            os.path.join(CALIBRATE_DATA_DIR, f"calstate-{chain_id}.pkl"), CALIBRATE_DATA_DIR
        )
