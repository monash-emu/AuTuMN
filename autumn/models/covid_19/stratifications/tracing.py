...
def get_tracing_strat(
    quarantine_infect_mult: float, other_infect_mults: Dict[str, float]
) -> Stratification:
    """
    ...
    tracing_strat = Stratification(
        "tracing", [Tracing.TRACED, Tracing.UNTRACED], DISEASE_COMPARTMENTS
    )

    # Everyone starts out untraced
    tracing_strat.set_population_split({Tracing.TRACED: 0.0, Tracing.UNTRACED: 1.0})

    # Everybody starts out untraced when they are first infected
    tracing_strat.set_flow_adjustments(
        INFECTION, {Tracing.TRACED: Multiply(0.0), Tracing.UNTRACED: Multiply(1.0)}
    )

    # Ensure the infectiousness adjustments are the same, which ensures that the stratification order doesn't matter
    multipliers_equal = [
        quarantine_infect_mult == other_infect_mults[mult]
        for mult in other_infect_mults
    ]
    ...
    for compartment in DISEASE_COMPARTMENTS:
        traced_infectious_adj = {
            Tracing.TRACED: Overwrite(quarantine_infect_mult),
