...
def get_vaccination_strat(
    ...
    pop_split = {stratum: 0.0 for stratum in all_strata}
    pop_split[Vaccination.UNVACCINATED] = 1.0
    stratification.set_population_split(pop_split)

    # Immunity adjustments equivalent to history approach
    apply_immunity_to_strat(
        stratification, params, stratified_adjusters, Vaccination.UNVACCINATED
    )
    ...
    if params.voc_emergence:
        for voc_name, voc_values in params.voc_emergence.items():
            seed_split = {stratum: Multiply(0.0) for stratum in all_strata}
            seed_split[Vaccination.UNVACCINATED] = Multiply(1.0)
