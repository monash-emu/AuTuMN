...
from summer.parameters import Function
...
def get_compartments(sojourns: Sojourns) -> List[str]:
    """
    Find the model compartments that are applicable to the parameter requests, based on the sojourn
    times structure.
        - Start with just the base SEIR structure
        - Add in a second serial latent compartment if there is a proportion of the latent period
          early
        - Add in a second serial active compartment if there is a proportion of the active period
          early
...
def assign_population(builder: ModelBuilder, total_pop: int):
    """
    Assign the starting population to the model according to the user requests and total population
    of the model.
    ...
    def get_init_pop(total_pop: float, seed: float):
    ...
    # Now provide a computegraph.types.Function object
    # that describes where it gets its inputs from

    seed_param = builder.param("infectious_seed")

    init_pop = Function(get_init_pop, [total_pop, seed_param])

    # Assign to the model
    # Add an output to the model builder for this function
    builder.add_output("init_pop", init_pop)

    # Tell the mode to get it's initial population from this output
...
def add_sojourn_transitions(
    ...
    if early_prop:
        ...
        def get_progress_rate(total_time, proportion_early):
...
def apply_reinfection_flows_without_strains(
    ...
    """
    Apply the reinfection flows in the case of a single-strain model. Note that in this case, only
    the late reinfection flow (i.e. coming out of the waned compartment) is relevant.
    ...
    Args:
        ...
        suscept_props: Adjustments to the rate of infection of susceptibles based on modelled age
                       groups
...
def build_model(params: dict, build_options: dict = None) -> CompartmentalModel:
    ...
    builder = ModelBuilder(model, params_dict, params)
    ...
    assign_population(builder, age_pops.sum())
    ...
    if type(suscept_req) == dict:
        ...
        suscept_adjs = suscept_req  # In which case it should be None or a float, confirmed in
    ...
    # Apply the clinical stratification, or a None to indicate no clinical stratification to get a
    ...
    if voc_params:
        # The code should run fine if VoC parameters have been submitted but the strain
        # stratification hasn't been implemented yet - but at this stage we assume we
        # don't want it to
