...
def get_check_all_prop(name):
    ...
    msg = (
        f"Parameter '{name}' contains values outside [0, 1], but is intended "
        "as a list of proportions"
    )
...
def get_check_all_dict_values_non_neg(name):
    ...
    msg = (
        f"Dictionary parameter '{name}' contains negative values, but is intended as a "
        "list of proportions"
    )
...
class Country(BaseModel):
    ...
    The country may be, and often is, the same as the region.
    """
...
class AgeSpecificProps(BaseModel):
    ...
    def check_source_distribution(vals):
        sum_of_values = sum(vals.values())
        msg = (
            f"Proportions by immunity status in source for parameters does not sum to one: "
            "{sum_of_values}"
        )
    ...
    def check_source_protection(protection_params):
        ...
        assert all([0.0 <= val <= 1.0 for val in protection_params.values()]), msg
...
class TestingToDetection(BaseModel):
    ...
    def check_floor_request(cls, values):
        ...
        msg = (
            f"Requested value for the CDR floor does not fall between zero and the assumed CDR "
            "parameter of {assumed_cdr}, value is: {floor_value}"
        )
...
class VocComponent(BaseModel):
    ...
    def check_starting_strain_multiplier(cls, values):
        if values["starting_strain"]:
            ...
            msg = (
                f"Starting or 'wild type' strain must have a contact rate multiplier of one: "
                "{multiplier}"
            )
...
class Parameters:
    ...
    mobility: Mobility
    detect_prop: float  # Not optional, so as always to have a back-up value available if testing
    # to detection not used
    ...
    def validate_age_groups(age_groups):
        msg = (
            "Not all requested age groups in the available age groups of 5-year increments from "
            "zero to 75"
        )
        int_age_groups = [int(i_group) for i_group in COVID_BASE_AGEGROUPS]
        assert all([int(i_group) in int_age_groups for i_group in age_groups]), msg
        return age_groups
