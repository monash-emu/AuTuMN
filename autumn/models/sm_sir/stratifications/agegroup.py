...
def get_relevant_indices(
    ...
    for i_age, model_agegroup in enumerate(model_groups):
        ...
        relevant_indices[str(model_agegroup)] = standard_breaks[
            age_index_low:age_index_up
        ]

    # Should be impossible for this check to fail
    msg = "Not all source age groups being mapped to modelled age groups"
    assert (
        list(itertools.chain.from_iterable(relevant_indices.values()))
        == standard_breaks
    ), msg
...
def convert_param_agegroups(
    ...
    total_pops_5year_dict = {
        age: pop for age, pop in zip(source_agebreaks, total_pops_5year_bands)
    }

    msg = "Modelled age group(s) incorrectly specified, not in standard age breaks"
    assert all(
        [int(age_group) in source_agebreaks for age_group in modelled_age_groups]
    ), msg

    # Find out which of the standard source categories (values) apply to each modelled age group (keys)
    relevant_source_indices = get_relevant_indices(
        source_agebreaks, modelled_age_groups
    )
