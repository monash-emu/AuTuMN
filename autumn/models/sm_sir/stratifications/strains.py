...
from summer.parameters import Function, Time

from autumn.models.sm_sir.constants import Compartment, FlowName
from autumn.models.sm_sir.parameters import VocComponent, VocSeed
from autumn.model_features.strains import broadcast_infection_flows_over_source

from autumn.model_features.functional import windowed_constant
from autumn.core.model_builder import ModelBuilder


def make_voc_seed_func(builder: ModelBuilder, seed_params: VocSeed) -> Function:
    """
    Create a simple step function to allow seeding of the VoC strain for the period from the
    requested time point.
    The function starts from zero, steps up to entry_rate from the start_time and then steps back
    down again to zero after seed_duration has elapsed.
    ...
    def voc_seed_func(time: float, start_time: float, entry_rate: float, seed_duration: float):
        return windowed_constant(time, entry_rate, start_time, seed_duration)

    seed_paramfunc = builder.get_mapped_func(voc_seed_func, seed_params, kwargs={"time": Time})

    return seed_paramfunc


def seed_vocs(
    builder: ModelBuilder, all_voc_params: Dict[str, VocComponent], seed_compartment: str
):
    """
    Use importation flows to seed VoC cases.

    Generally seeding to the infectious compartment, because unlike Covid model, this compartment
    is always present.

    Note that the entry rate will get repeated for each compartment as the requested compartments
    for entry are progressively stratified after this process is applied (but are split over the
    previous stratifications of the compartment to which this is applied, because the split_imports
    argument is True).
    ...
    for voc_name, this_voc_params in all_voc_params.items():
        ...
        if voc_seed_params:
            voc_seed_func = make_voc_seed_func(builder, voc_seed_params)
...
def apply_reinfection_flows_with_strains(
    ...
    """
    Apply the reinfection flows, making sure that it is possible to be infected with any strain
    after infection with any strain. We'll work out whether this occurs at a reduced rate because
    of immunity later, in the various functions of the immunity.py file.
    ...
    Args:
        ...
        strain_strata: The strains being implemented or a list of an empty string if no strains in
                       the model
        contact_rate: The model's contact rate
        suscept_adjs: Adjustments to the rate of infection of susceptibles based on modelled age
                      groups
    ...
    for dest_strain in strain_strata:
        ...
        for age_group in age_groups:
            ...
            # Get an adjuster that considers both the relative infectiousness of the strain and the
            # relative susceptibility of the age group
...
def get_strain_strat(voc_params: Optional[Dict[str, VocComponent]], compartments: List[str]):
    """
    Stratify the model by strain, with at least two strata, being wild or "ancestral" virus type
    and the variants of concern ("VoC").

    We are now stratifying all the compartments, including the recovered ones. The recovered
    compartment stratified by strain represents people whose last infection was with that strain.
    ...
    for strain in strains:
        if voc_params[strain].relative_latency:
            adjustments_latent.update({strain: Multiply(1.0 / voc_params[strain].relative_latency)})
    ...
    if Compartment.LATENT_LATE in compartments:
        strain_strat.set_flow_adjustments(FlowName.WITHIN_LATENT, adjustments=adjustments_latent)
    if Compartment.LATENT in compartments:
        strain_strat.set_flow_adjustments(FlowName.PROGRESSION, adjustments=adjustments_latent)
    ...
    if Compartment.INFECTIOUS in compartments:
        strain_strat.set_flow_adjustments(FlowName.RECOVERY, adjustments=adjustments_active)
