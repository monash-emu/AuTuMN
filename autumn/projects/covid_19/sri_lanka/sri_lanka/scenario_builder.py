...
lockdown_title = [
    "No lockdowns placed",
    "What if lockdown was initiated from April 21 - June 21",
    ...
    for i_age_min, age_min in enumerate(age_mins):
        ...
        component = {
            "supply_period_coverage": {
                "coverage": coverage[i_age_min],
                "start_time": start_time[i_age_min],
                "end_time": end_time[i_age_min],
            },
        }
...
def get_all_scenario_dicts(country: str):
    ...
    for i_lockdown_scenario in [*range(0, num_scenarios)]:
        ...
        scenario_dict = {
            ...
            + f"{i_lockdown_scenario+1}"
            + f" {lockdown_title[i_lockdown_scenario]}",
            "mobility": {"mixing": {}},
        ...
        if (
            i_lockdown_scenario == 1
        ):  # What if lockdown was initiated from April 21 - June 21

            # lockdown mobility from 21May -21 June
            times1, values1 = get_mobility_specific_period(
                country,
                None,
                {
                    "work": {"workplaces": 1.0},
            ...
            times1 = [
                *range(477, 507)
            ]  # lockdown values from May 21 - June 21 is assigned from April 21 -May 20

            # from May 20 - 21 June, the average mobility from from May 21 - June 21
            times5 = [*range(507, 539)]
            values5 = {
                "work": [0.4] * len(times5),
                "other_locations": [0.4] * len(times5),
            }

            # mobility from 22 June -21st August
            times2, values2 = get_mobility_specific_period(
                country,
                None,
                {
                    "work": {"workplaces": 1.0},
            ...
            values3 = {
                "work": [0.65] * len(times3),
                "other_locations": [0.8] * len(times3),
            }

            # In the scenarios applying the actual values observed from Oct 02 -Oct 12 (after lockdown)
            times4, values4 = get_mobility_specific_period(
                country,
                None,
                {
                    "work": {"workplaces": 1.0},
        ...
        if (
            ...
            values3 = {
                "work": [0.4] * len(times3),
                "other_locations": [0.4] * len(times3),
            }
        ...
        scenario_dict["vaccination"]["roll_out_components"] = get_vaccine_roll_out(
            i_lockdown_scenario
        )
