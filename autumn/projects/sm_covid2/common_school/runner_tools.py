...
from autumn.projects.sm_covid2.common_school.parallel import map_parallel
...
from autumn.projects.sm_covid2.common_school.calibration_plots.opti_plots import (
    plot_opti_params,
    plot_model_fit,
    plot_multiple_model_fits,
)
...
def sample_with_lhs(n_samples, bcm):
    ...
    sample_as_dicts = [
        {p: sample[i][j] for j, p in enumerate(lhs_sampled_params)} for i in range(n_samples)
    ]
...
def multi_country_optimise(
    iso3_list: list,
    analysis: str = "main",
    num_workers: int = 8,
    search_iterations: int = 7000,
    parallel_opti_jobs: int = 4,
    logger=getLogger("multi_country_optimise"),
    out_path: str = None,
):
    ...
    best_params_dict = {k: v for (k, v) in best_params_iter}
    ...
    map_parallel(plot_wrapper, iso3_list)
...
def get_sampled_results(sampled_df, output_names):
    ...
    sampled_results = {
        output: pd.DataFrame(index=bcm.model._get_ref_idx(), columns=d2_index)
...
def run_full_runs(sampled_df, iso3, analysis):
    ...
    scenario_params = {
        "baseline": {},
        "scenario_1": {
            "mobility": {"unesco_partial_opening_value": 1.0, "unesco_full_closure_value": 1.0}
        },
    }
...
def diff_latest_output(outputs_df_latest_0, outputs_df_latest_1, column, relative=False):
    if not relative:
        ...
        return (outputs_df_latest_0[column] - outputs_df_latest_1[column]) / outputs_df_latest_1[
            column
        ]
...
def calculate_diff_outputs(outputs_df):
    ...
    diff_outputs_df["cases_averted"] = diff_latest_output(
        outputs_df_latest_0, outputs_df_latest_1, "cumulative_incidence"
    )
    diff_outputs_df["cases_averted_relative"] = diff_latest_output(
        outputs_df_latest_0, outputs_df_latest_1, "cumulative_incidence", relative=True
    )

    diff_outputs_df["deaths_averted"] = diff_latest_output(
        outputs_df_latest_0, outputs_df_latest_1, "cumulative_infection_deaths"
    )
    diff_outputs_df["deaths_averted_relative"] = diff_latest_output(
        outputs_df_latest_0, outputs_df_latest_1, "cumulative_infection_deaths", relative=True
    )

    diff_outputs_df["delta_student_weeks_missed"] = diff_latest_output(
        outputs_df_latest_0, outputs_df_latest_1, "student_weeks_missed"
    )

    diff_outputs_df["delta_hospital_peak"] = diff_latest_output(
        outputs_df_latest_0, outputs_df_latest_1, "peak_hospital_occupancy"
    )
    diff_outputs_df["delta_hospital_peak_relative"] = diff_latest_output(
        outputs_df_latest_0, outputs_df_latest_1, "peak_hospital_occupancy", relative=True
    )

    return diff_outputs_df


def get_quantile_outputs(outputs_df, diff_outputs_df, quantiles=[0.025, 0.25, 0.5, 0.75, 0.975]):
    ...
    diff_quantiles_df = pd.DataFrame(
        index=quantiles,
        data={col: np.quantile(diff_outputs_df[col], quantiles) for col in diff_outputs_df.columns},
    )
...
def run_full_analysis(
    ...
    def opti_func(sample_dict):
        best_p, _ = optimise_model_fit(
            bcm,
            num_workers=opti_params["num_workers"],
            warmup_iterations=opti_params["warmup_iterations"],
            search_iterations=opti_params["search_iterations"],
            suggested_start=sample_dict,
        )
        return best_p

    best_params = map_parallel(
        opti_func, sample_as_dicts, n_workers=opti_params["parallel_opti_jobs"]
    )
    ...
    if logger:
        logger.info(
            f"Start MCMC for {mcmc_params['tune']} + {mcmc_params['draws']} iterations and {mcmc_params['chains']} chains..."
        )
    ...
    idata = sample_with_pymc(
        bcm,
        initvals=init_vals,
        draws=mcmc_params["draws"],
        tune=mcmc_params["tune"],
        cores=mcmc_params["cores"],
