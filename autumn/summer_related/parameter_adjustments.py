...
def adjust_upstream_stratified_parameter(
    ...
    implement_strata = change_parameters_to_overwrite(implement_strata) if overwrite else implement_strata

    # Loop over upstream strata
    param_adjustments = {}
    for i_upstream_stratum, upstream_stratum in enumerate(upstream_strata):
        sub_param_to_adjust = base_parameter + 'X' + upstream_stratification + '_' + upstream_stratum
        ...
        for i_stratum, stratum in enumerate(implement_strata):
            param_adjustments[sub_param_to_adjust].update(
                {
                    stratum:
                        parameter_values[i_stratum][i_upstream_stratum]
                }
            )
...
def split_prop_into_two_subprops(overall_props, overall_prop_name, split_props, split_prop_name):
    ...
    return {
        split_prop_name:
            [overall_props[i_agegroup] * split_props[i_agegroup] for
             i_agegroup in range(len(split_props))],
        overall_prop_name + extra_underscore + 'non_' + split_prop_name:
            [overall_props[i_agegroup] * (1. - split_props[i_agegroup]) for
             i_agegroup in range(len(split_props))]
    }
