...
class Calibration:
    ...
    def tune_proposal(
        ...
        project: Project,
        n_points=100,
        relative_likelihood_reduction=0.5,
    ):
        assert (
        ...
        assert (
            n_points > 1
        ), "A minimum of two points is required to perform proposal tuning"
        ...
        prior_dict = [
            p_dict for p_dict in self.all_priors if p_dict["param_name"] == param_name
        ][0]
        ...
        eval_points = list(
            np.linspace(
                start=lower_bound, stop=upper_bound, num=n_points, endpoint=True
            )
        )
        ...
        return tune_jumping_stdev(
            eval_points, eval_log_postertiors, relative_likelihood_reduction
        )
    ...
    def run(
        ...
        target_names = [t.data.name for t in self.targets]
        self.derived_outputs_whitelist = list(
            set(target_names + derived_outputs_to_plot)
        )
        ...
        self.output = CalibrationOutputs(
            chain_idx, project.model_name, project.region_name
        )
        ...
        self.mcmc_trace_matrix = (
            None  # will store the results of the MCMC model calibration
        )

        if self.chain_idx == 0:
            plots.calibration.plot_pre_calibration(
                self.all_priors, self.output.output_dir
            )
    ...
    def run_model_with_params(self, proposed_params: dict):
        ...
        iter_params = self.model_parameters.update(
            ...
            if self.run_mode == CalibrationMode.AUTUMN_MCMC:
                if target.loglikelihood_distri in ["normal", "trunc_normal"]:
                    ...
                    if key + "_dispersion_param" in all_params_dict:
                        ...
                        raise ValueError(
                            f"A dispersion_param is required for target {key}"
                        )
                    ...
                    for i in range(len(data)):
                        ...
                        p = mu / (mu + n)
                        ll += (
                            stats.nbinom.logpmf(round(data[i]), n, 1.0 - p)
                            * time_weights[i]
                        )
                else:
                    raise ValueError(
    ...
    def workout_unspecified_jumping_stdevs(self):
        for i, prior_dict in enumerate(self.iterative_sampling_priors):
            if "jumping_stdev" not in prior_dict.keys():
                prior_low, prior_high = get_parameter_finite_range_from_prior(
                    prior_dict
                )
                ...
                self.iterative_sampling_priors[i]["jumping_stdev"] = (
                    relative_prior_width
                    * prior_width
                    * self.initial_jumping_stdev_ratio
                )
    ...
    def run_autumn_mcmc(self, available_time):
        ...
        self.mcmc_trace_matrix = (
            None  # will store param trace and loglikelihood evolution
        )
        ...
        self.n_accepted = 0
        self.n_iters_real = (
            0  # Actual number of iterations completed, as opposed to run_num.
        )
    ...
    def resume_autumn_mcmc(
        self, available_time: int = None, max_iters: int = None, finalise=True
    ):
        try:
    ...
    def enter_mcmc_loop(self, available_time: int = None, max_iters: int = None):
        ...
        while True:
            logging.info(
                "Running MCMC iteration %s, run %s", self.n_iters_real, self.run_num
            )

            # Not actually LHS sampling - just sampling directly from prior.
            independent_samples = draw_independent_samples(
                self.independent_sampling_priors
            )
            ...
            proposed_iterative_params = self.get_original_params(
                proposed_iterative_params_trans
            )
            ...
            if is_within_prior_support:
                ...
                for i, prior_dict in enumerate(
                    ...
                    inv_derivative = self.transform[prior_dict["param_name"]][
                        "inverse_derivative"
            ...
            if accept:
                self.last_accepted_iterative_params_trans = (
                    proposed_iterative_params_trans
                )
            ...
            logging.info(
                "Finished MCMC iteration %s, run %s", self.n_iters_real, self.run_num
            )
            self.run_num += 1
            self.n_iters_real += 1
            if available_time:
            ...
            if self.adaptive_proposal and self.run_num == self.n_steps_fixed_proposal:
                ...
                if acceptance_ratio < ADAPTIVE_METROPOLIS["MIN_ACCEPTANCE_RATIO"]:
                    logger.info(
                        "Acceptance ratio too low, restart sampling from scratch."
                    )
                    (
                        self.run_num,
                        self.n_accepted,
    ...
    def reduce_proposal_step_size(self):
        ...
        """
        for i in range(len(self.iterative_sampling_priors)):
            self.iterative_sampling_priors[i][
                "jumping_stdev"
            ] *= self.jumping_stdev_adjustment
    ...
    def build_adaptive_covariance_matrix(self, haario_scaling_factor):
        ...
        adaptive_cov_matrix = (
            scaling_factor * cov_matrix
            + scaling_factor
            * ADAPTIVE_METROPOLIS["EPSILON"]
    ...
    def get_original_params(self, transformed_iterative_params):
        ...
        for i, prior_dict in enumerate(self.iterative_sampling_priors):
            original_iterative_params.append(
                self.transform[prior_dict["param_name"]]["inverse"](
                    transformed_iterative_params[i]
                )
            )
        ...
        if not use_adaptive_proposal:
            for i, prior_dict in enumerate(self.iterative_sampling_priors):
                sample = np.random.normal(
                    loc=prev_iterative_params_trans[i],
                    scale=prior_dict["jumping_stdev"],
                    size=1,
                )[0]
    ...
    def logprior(self, all_params_dict):
        ...
        for param_name, value in all_params_dict.items():
            prior_dict = [d for d in self.all_priors if d["param_name"] == param_name][
                0
            ]
...
class CalibrationOutputs:
    ...
    def __init__(self, chain_id: int, app_name: str, region_name: str):
        ...
        project_dir = os.path.join(
            settings.OUTPUT_DATA_PATH, "calibrate", app_name, region_name
        )
        ...
        if os.path.exists(self.output_db_path):
            ...
            logger.info(
                "File found at %s, recreating %s", self.output_db_path, self.output_dir
            )
    ...
    if idx is not None:
        ...
        for target in target_outputs:
            first_idx_to_keep = next(
                t_idx
                for t_idx, t_val in enumerate(target.data.index)
                if t_val > latest_start_time
            )
...
def set_initial_point(
    ...
    if initialisation_type == MetroInit.LHS:
        ...
        raise ValueError(
            f"{initialisation_type} is not a supported Initialisation Type"
        )
...
def read_current_parameter_values(priors, model_parameters):
    ...
    for param_dict in priors:
        if param_dict["param_name"].endswith("dispersion_param"):
            ...
            starting_points[param_dict["param_name"]] = np.mean(
                param_dict["distri_params"]
            )
        else:
