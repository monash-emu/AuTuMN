...
def tune_jumping_stdev(eval_points, eval_logposteriors, relative_likelihood_reduction):
    ...
    """
    assert (
        0.0 < relative_likelihood_reduction < 1.0
    ), "relative_likelihood_reduction must be in (0, 1)"
    ...
    if max_loglikelihood < -100:
        print(
            "more parameter samples may be required to identify higher-likelihood regions"
        )
        return None
    ...
    for moving_direction in moving_directions:
        latest_over_index, latest_over_point, latest_over_val = (
            index_max,
            best_point,
            max_loglikelihood,
        )
        while True:
            ...
            if eval_value <= lower_threshold_loglikelihood:
                delta_y_ratio = (lower_threshold_loglikelihood - eval_value) / (
                    latest_over_val - lower_threshold_loglikelihood
                )
                sol = (eval_point + latest_over_point * delta_y_ratio) / (
                    1.0 + delta_y_ratio
                )
...
def perform_all_params_proposal_tuning(
    project, calibration, priors, n_points=100, relative_likelihood_reduction=0.2
):
    ...
    for i, param_name in enumerate(
        params_to_tune
    ):  # FIXME: these tasks should be run in parallel
        print(
            f"Completed {i} out of {len(params_to_tune)} tuned parameters. Now tuning {param_name} "
        )
        tuned_proposal_sds[param_name] = calibration.tune_proposal(
            param_name, project, n_points, relative_likelihood_reduction
        )
    ...
    print()
    print(
        f"Please paste the below code into the proposals_sds.yml file of project {project.region_name}."
    )
    print()
