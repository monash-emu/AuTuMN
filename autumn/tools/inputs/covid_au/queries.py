...
def get_dhhs_vaccination_numbers(cluster: str = None):
    """
    ...
    if cluster is None:
        df = input_db.query("covid_dhhs_victoria_2021", columns=["date_index", "dose_1"])
        df = df.groupby("date", as_index=False).sum()
    else:
        df = input_db.query(
            "covid_dhhs_victoria_2021", columns=["date_index", "dose_1"], conditions={"cluster_name": cluster}
        )
    date_str_to_int = lambda s: (datetime.strptime(s, "%Y-%m-%d") - COVID_BASE_DATETIME).days

    vac_dates = df.date.to_numpy()
    test_values = df.test.to_numpy()
    epsilon = 1e-6  # A really tiny number to avoid having any zeros
    avg_vals = np.array(apply_moving_average(test_values, 7)) + epsilon
