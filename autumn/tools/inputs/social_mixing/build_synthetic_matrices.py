...
def load_socialmixr_matrices(proxy_country_iso3, requested_locations):
    ...
    for i, location in enumerate(requested_locations):
        matrix_path = os.path.join(
            SOURCE_MATRICES_PATH, proxy_country_iso3, f"{location}.csv"
        )
        ...
        if i == 0:
            ...
            age_breaks = [
                age_group.split(",")[0].split("+")[0].replace("[", "")
                for age_group in col_names
            ]
        else:
...
def adjust_matrices_for_age_distribution(
    ...
    age_pop_ratio = [
        p_modelled / p_proxy
        for (p_modelled, p_proxy) in zip(
            age_proportions_modelled, age_proportions_proxy
        )
    ]
    ...
    for location in requested_locations:
        age_adjusted_matrices[location] = np.dot(
            source_matrices[location], diag_age_pop_ratio
        )
...
def find_source_age_group_contributions(
    ...
    for i_break, modelled_age_break in enumerate(modelled_breaks):
        model_lower, model_upper = (
            int(modelled_age_break),
            modelled_upper_bounds[i_break],
        )
        ...
        for j_source, source_age_break in enumerate(source_breaks):
            ...
            source_lower, source_upper = (
                int(source_age_break),
                source_upper_bounds[j_source],
            )
            if model_upper > source_lower and model_lower < source_upper:
                overlap_range = max(source_lower, model_lower), min(
                    source_upper, model_upper
                )
                contributions_array[
                    i_break, j_source
...
def convert_matrices_agegroups(
    ...
    for location in requested_locations:
        ...
        for i_model in range(n_modelled_groups):
            i_contributions = source_age_break_contributions[i_model, :].reshape(
                n_source_groups, 1
            )
...
def _get_pop_props_by_age(
    age_breaks, country_iso3, region_name=None, reference_year=2020
):
...
def _get_proportion_between_ages_among_agegroup(
    ...
    popsize_denominator = get_population_by_agegroup(
        [0, denominator_low, denominator_up],
        modelled_country_iso3,
        modelled_region_name,
        year=2020,
    )[1]
    popsize_numerator = get_population_by_agegroup(
        [0, numerator_low, numerator_up],
        modelled_country_iso3,
        modelled_region_name,
        year=2020,
    )[1]
...
def _check_model_ready_matrices(matrices):
    ...
    sum_contacts_by_location = (
        ...
        + matrices["school"]
        + matrices["work"]
        + matrices["other_locations"]
    )
    error = all_contacts - sum_contacts_by_location
    assert (
        abs(error.max()) < 1.0e-6
    ), "The sum of the 4 location-specific matrices should match the all_locations matrix"
