...
#######################################################################################
        ...
        if function == str('program_prop_vaccination'): #using data from data_fiji
            ...
            popsize = map(model.scaleup_fns['econ_program_popsize_vaccination'], x_vals)
            ...
            for i in numpy.arange(0, len(x_vals), 1):
                        ...
                        def F(a):
                            ...
                            result = a - saturation/(1-(1 + math.exp(-b*(reflection_cost_scaleup[int(i)]/2.-reflection_cost_scaleup[int(i)])))**alpha)
                        ...
                        aa.append(a)
                        print(len(aa))
                        b_growth_rate.append((2**(alpha + 1)) / (alpha * (saturation - a) * unitcost[int(i)] * popsize[int(i)]))
