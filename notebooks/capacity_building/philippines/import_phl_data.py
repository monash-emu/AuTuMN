...
def get_fassster_data(link: str) -> pd.DataFrame:
    ...
    filename = [each for each in zipfile_ob.namelist() if each.startswith("2022")]
...
def get_population_and_epi_data(phl_doh_link: str, phl_fasster_link: str) -> (int, pd.DataFrame):
    ...
    df_cases = df_cases.groupby(
        ["Report_Date", "Region"], as_index=False
    ).size()  # Because each row is a case we can use the group size.
    df_cases = df_cases[df_cases["Region"] == "NCR"]  # Filter for NCR cases
    df_cases = df_cases.rename(
        columns={"Report_Date": "reportdate", "size": "cases"}
    )  # Rename columns to match DoH names.
    ...
    initial_population = get_ncr_population()

    return initial_population, df


def get_ncr_population():
    ...
    df_pop = df_pop.melt(
        id_vars=["country", "iso3", "region", "year"], var_name="age_group", value_name="pop"
    )
    df_pop = df_pop[["region", "pop"]].groupby("region", as_index=False).sum()
    initial_population = (
        df_pop["pop"][0] * 1000
    )  # We need to multiply by 1000 to covert it back to counts.
    return initial_population


def get_timeseries_data():
    timeseries_url = "https://raw.githubusercontent.com/monash-emu/AuTuMN/master/autumn/projects/sm_sir/philippines/national-capital-region/timeseries.json"

    df = pd.read_json(timeseries_url)
    times = df["notifications"][2]
    times = [datetime(2019, 12, 31) + timedelta(days=t) for t in times]
    values = df["notifications"][3]

    df = pd.DataFrame({"cases": values}, index=times)
    df.index.rename("reportdate", inplace=True)
    initial_population = get_ncr_population()
    return initial_population, df
