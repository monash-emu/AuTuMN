...
import pymc3 as pm
#import arviz as az
#from pymc3 import Uniform, Beta, Model, Deterministic, Normal
import datetime
import theano
...
def unc_run(theta):
     ...
     #print(start_time.tag.test_value)
...
def tb_loglike(theta, data, sigma ):
    model = unc_run(theta)
    print(model)
    print(-(0.5/sigma**2)*np.sum((data - model)**2))
...
def mcmc(prior_distribution, outputs_unc, step_method_param='tb_n_contact'):
    ...
    sigma = 1.
    # create our Op
    logl = LogLike(tb_loglike, data, sigma)
    ...
    with pm.Model() as unc_model:
        ...
        print('-----------------------------')
        #prev = pm.Deterministic('prev',unc_run(beta, cdr_adjustment, start_time))

        #prev = tt.printing.Print('prev')(prev)
        ...
        #y = pm.Normal('y', mu=prev, tau=1000000, observed=0.006 )
        #y = tt.printing.Print('y')(y)


        step = pm.Metropolis()
        trace = pm.sample(150, step, tune=0, chains=1,   progressbar=False)
        pm.summary(trace)
