...
ALL_SCHOOL_ISO3S = list(country_dict["all"].keys())

@pytest.mark.local_only
@pytest.mark.parametrize("iso3", ISO3S_FOR_TEST_RUNS)
def test_run_models_partial(iso3):
    """
    Smoke test: ensure we can build and run each default model with nothing crashing.
    Does not include scenarios, plotting, etc.
    """
    project = get_project(Models.SM_COVID2, iso3)
...
@pytest.mark.parametrize("iso3", ISO3S_FOR_TEST_RUNS)  # ALL_SCHOOL_REGIONS)
def test_build_scenario_models(iso3):
    """
