...
def test_scale_up_function(verify, method, func_name, func):
    ...
    scaled_func = scale_up_function(
        x=INPUT_TIMES, y=[func(t) for t in INPUT_TIMES], method=method
    )
        ...
        for inflect_time in (0.0, 100.0, 500.0):
            for start_asymptote in (0.0, 1.0):
                for end_asymptote in (1.0, 100.0):
                    ...
                    # Get the results
                    results = [
                        tanh_function(i_time) for i_time in np.linspace(0.0, 100.0, 10)
                    ]
                    ...
                    assert (
                        inflection_result == start_asymptote / 2.0 + end_asymptote / 2
                    )
